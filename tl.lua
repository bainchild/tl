...
local os_getenv = os.getenv or function(s) s = s; return nil end
local TL_DEBUG = os_getenv("TL_DEBUG")
local TL_DEBUG_FACTS = os_getenv("TL_DEBUG_FACTS")
...
function tl.generate(ast, gen_target, opts)
   ...
   local function convert_to_luau_type(typ, stack)
      for _, name in ipairs(stack) do if typ.typename == name then return "unknown" end end
      if typ.typename == "record" then
         table.insert(stack, 1, typ.typename)
         local out = { "{" }
         local i = 0
         for fname, ftype in fields_of(typ) do
            if ftype.typename ~= typ.typename then
               if i > 0 then
                  table.insert(out, ",")
               end
               i = i + 1
               ...
               table.insert(out, ": ")
               table.insert(out, convert_to_luau_type(ftype, stack))
            end
         end
      ...
      if typ.typename == "nominal" and (typ.resolved == nil or typ.typename ~= typ.resolved.typename) then
         return typ.resolved and convert_to_luau_type(typ.resolved, stack) or "unknown"
      end
      if typ.typename == "integer" then return "number" end
      return typ.typename
   end

   local function print_record_def(typ, gen_u)
      local out = { "{" }
      local i = 0
      for fname, ftype in fields_of(typ) do
         if ftype.typename == "typedecl" then
            ...
            if def.typename == "record" then
               ...
               table.insert(out, print_record_def(def, gen_u))
            end
         elseif gen_u and (ftype.elements or ftype.typename == "string" or is_numeric_type(ftype) or ftype.typename == "boolean") then
            if i > 0 then
               table.insert(out, ",")
            end
            i = i + 1
            ...
            if ftype.elements then
               table.insert(out, "{")
               table.insert(out, convert_to_luau_type(ftype.elements, {}))
               table.insert(out, "}")
            elseif ftype.typename == "string" or is_numeric_type(ftype) or ftype.typename == "boolean" then
               table.insert(out, convert_to_luau_type(ftype, {}))
            end
         end
      end
      if i > 0 then
   ...
   visit_node.cbs = {
      ...
      ["newtype"] = {
         after = function(_, node, _children)
            ...
            if nt.typename == "typedecl" then
               local def = nt.def
               if def.fields then
                  table.insert(out, print_record_def(def, false))
      ...
      ["string"] = {
         after = function(_, node, children)
            ...
            if node.tk:sub(1, 1) == "[" or (gen_target ~= "5.1" and gen_target ~= "U") or not node.tk:find("\\", 1, true) then
...
tl.search_module = function(module_name, search_all)
   ...
   local fd
   local tried = {}
   local path = os_getenv("TL_PATH") or tl.path or package.path
   if search_all then
...
local function add_compat_entries(program, used_set, gen_compat, gen_target)
...
self:expand_type(node, values, elements) })
      ...
      ["op"] = {
         ...
         after = function(self, node, children)
            ...
            if node.op.arity == 1 and unop_types[node.op.op] then
               ...
               if node.op.op == "~" and (self.gen_target == "5.1" or self.gen_target == "U") then
            ...
            if node.op.arity == 2 and binop_types[node.op.op] then
               ...
               elseif bit_operators[node.op.op] and (self.gen_target == "5.1" or self.gen_target == "U") then
                  if meta_on_operator then
                     self.all_needs_compat["mt"] = true
   ...
   tl.check = function(ast, filename, opts, env)
      ...
      add_compat_entries(ast, self.all_needs_compat, self.gen_compat, self.gen_target)
...
tl.init_env = function(lax, gen_compat, gen_target, predefined)
   local opts = {
      defaults = {
         ...
         (_VERSION:sub(1, 5) == "Luau " and "U") or
