...
tl.type_check = function(ast, opts)
   ...
   is_a = function(t1, t2, for_equality)
      ...
      elseif t1.typename == "nominal" and t2.typename == "nominal" then
         ...
         if is_record_type(t1r) and is_record_type(t2r) then
            return same, err
         elseif t1r.typename == "union" then
            return is_a(t1r, t2, for_equality)
         elseif t2r.typename == "union" then
            return is_a(t1, t2r, for_equality)
         else
   ...
   local function check_metamethod(node, method_name, a, b)
   ...
   local function match_record_key(tbl, rec, key)
      ...
      if is_record_type(tbl) then
         ...
         local meta_t = check_metamethod(rec, "__index", tbl, STRING)
         if meta_t then
            return meta_t
         end
   ...
   local function type_check_index(anode, bnode, a, b)
      ...
      local meta_t = check_metamethod(anode, "__index", a, orig_b)
      if meta_t then
         return meta_t
      end
         ...
         after = function(node, children)
            ...
            elseif node.op.op == "is" then
               ...
               elseif node.e1.kind == "variable" then
                  check_metamethod(node, "__is", ra, resolve_typetype(rb))
            ...
            elseif node.op.arity == 1 and unop_types[node.op.op] then
               ...
               if not node.type then
                  ...
                  if mt_name then
                     node.type, meta_on_operator = check_metamethod(node, mt_name, a)
                  end
                  if not node.type then
            ...
            elseif node.op.arity == 2 and binop_types[node.op.op] then
               ...
               if not node.type then
                  ...
                  if mt_name then
                     node.type, meta_on_operator = check_metamethod(node, mt_name, a, b)
                  end
                  if not node.type then
